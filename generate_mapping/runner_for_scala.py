...
class Runner:
    ...
    def skipTrace(self, trace):
        ...
        if self.module == "spark-core":
            if "org.apache.spark.SparkConf" in trace and "Test" not in trace:
                return True
        return False
        ...
        for line in lines:
            ...
            if "[CTEST][GET-PARAM]" in line:
                ...
                if param_name in self.params:
                    is_getter = True 
                    self.getter_record.write(method + " param: " + param_name + "\n")
    ...
    def run_individual_testmethod(self):
        ...
        for method in all_test_methods:
            ...
            corrected_name= method.split('.')[-1].replace("/", "")
            log_file_name = out_dir + corrected_name + "-log.txt"
            print(corrected_name)
            method_out = open(log_file_name, "w+")
            method_report_path = report_dir + corrected_name + "-report.txt"
